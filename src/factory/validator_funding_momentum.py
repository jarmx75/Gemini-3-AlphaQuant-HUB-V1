"""
Factory Validator for FUNDING_MOMENTUM_1H
Ejecuta backtesting Walk-Forward sobre BTCUSDT y ETHUSDT usando datos reales de Funding Rate y Momentum:
...
from src.strategies.funding_momentum_1h import FundingMomentum1H
from src.factory.generator_funding_momentum import FundingMomentumCandidate

@dataclass
class FundingMomentumEvaluationResult:
    candidate_id: str
    family: str
    train_pf: float
    test_pf: float
...
class FundingMomentumValidator:
    """Validador Walk-Forward para Funding Momentum."""
    ...
    def load_and_prepare(self):
        ...
        for sym in self.symbols:
            ...
            if ohlcv_file.exists() and funding_file.exists():
                ...
                # Instanciar con momentum base 12h para preprocesamiento de series
                engine = FundingMomentum1H(momentum_hours=12)
    ...
    def simulate_asset(
        ...
        cand: FundingMomentumCandidate,
        notional: float = 300.0
    ) -> List[Dict[str, Any]]:
        df = df_prepared.copy()
        # Recalcular mom_ret si cand.momentum_hours != 12
        if cand.momentum_hours != 12:
            df['mom_ret'] = df['close'].pct_change(cand.momentum_hours).shift(1).fillna(0.0)
            
        opens = df['open'].values
        closes = df['close'].values
        is_funding = df['is_funding_bar'].values
        funding_z = df['funding_z'].values
        funding_z_held = df['funding_z_held'].values
        mom_ret = df['mom_ret'].values
        n = len(df)
        ...
        start_idx = max(cand.momentum_hours, 100)
        ...
        for t in range(start_idx, n):
            ...
            if not in_pos:
                # Entrada en open de t si en t-1 hubo publicación de funding con momentum alineado
                if is_funding[t-1]:
                    prev_fz = funding_z[t-1]
                    prev_mom = mom_ret[t-1]
                    
                    # 1. Señal LONG: Funding positivo extremo + momentum positivo
                    if prev_fz >= cand.funding_z and prev_mom > 0:
                        in_pos = True
                        pos_side = 'LONG'
                        entry_price = curr_open
                        entry_idx = t
                    # 2. Señal SHORT: Funding negativo extremo + momentum negativo
                    elif prev_fz <= -cand.funding_z and prev_mom < 0:
                        in_pos = True
                        pos_side = 'SHORT'
                        entry_price = curr_open
                        entry_idx = t
                ...
                curr_fz = funding_z_held[t-1]
                curr_mom = mom_ret[t-1]
                
                if pos_side == 'LONG':
                    pnl_pct = (curr_close - entry_price) / entry_price
                    # Salida por neutralidad de funding o inversión de momentum
                    if curr_fz <= 0.0 or curr_mom < 0:
                        exit_flag = True
                        exit_price = curr_close
                        exit_reason = "Funding/Momentum Neutralized"
                    # Stop de Emergencia 3%
                    elif pnl_pct <= -0.03:
                        exit_flag = True
                        exit_price = curr_close
                        exit_reason = "Emergency Stop 3%"
                    # Time-Stop 24 velas (24h)
    ...
    def validate_candidate(self, cand: FundingMomentumCandidate) -> FundingMomentumEvaluationResult:
        ...
        return FundingMomentumEvaluationResult(
