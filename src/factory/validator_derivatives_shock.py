"""
Factory Validator for LIQUIDATION_DERIVATIVES_REVERSAL (DERIVATIVES_SHOCK_REVERSAL)
Ejecuta backtesting Walk-Forward sobre BTCUSDT y ETHUSDT usando datos reales de derivados:
- OHLCV 1H + Open Interest 1H + Taker Imbalance 1H + Funding Rate 8H
...
from src.strategies.derivatives_shock_reversal_1h import DerivativesShockReversal1H
from src.factory.generator_derivatives_shock import DerivativesShockCandidate

@dataclass
class DerivativesShockEvaluationResult:
    candidate_id: str
    family: str
    train_pf: float
    test_pf: float
...
class DerivativesShockValidator:
    """Validador Walk-Forward para Reversión tras Shock de Derivados."""
    ...
    def load_and_prepare(self):
        """Carga y fusiona datasets de OHLCV y derivados."""
        engine = DerivativesShockReversal1H()
        for sym in self.symbols:
            ohlcv_file = self.ohlcv_dir / f"{sym}_1h_2022_2026.csv"
            metrics_file = self.deriv_dir / f"{sym}_metrics_1h_2022_2026.csv"
            funding_file = self.deriv_dir / f"{sym}_funding_rate_2022_2026.csv"
            
            if ohlcv_file.exists() and metrics_file.exists() and funding_file.exists():
                df_ohlcv = pd.read_csv(ohlcv_file)
                df_metrics = pd.read_csv(metrics_file)
                df_funding = pd.read_csv(funding_file)
                
                df_prepared = engine.prepare_dataset(df_ohlcv, df_metrics, df_funding)
                self.prepared_data[sym] = df_prepared

    def simulate_asset(
        self,
        df_prepared: pd.DataFrame,
        cand: DerivativesShockCandidate,
        notional: float = 300.0
    ) -> List[Dict[str, Any]]:
        opens = df_prepared['open'].values
        closes = df_prepared['close'].values
        z_ret = df_prepared['z_ret'].values
        z_oi = df_prepared['z_oi'].values
        z_taker = df_prepared['z_taker'].values
        z_funding = df_prepared['z_funding'].values
        ...
        for t in range(start_idx, n):
            prev_zr = z_ret[t-1]
            prev_zoi = z_oi[t-1]
            prev_ztk = z_taker[t-1]
            prev_zfd = z_funding[t-1]
            ...
            if not in_pos:
                # 1. Señal LONG: Retorno extremadamente bajista + Shock de OI + Taker Sell masivo + Funding no alcista
                if (prev_zr <= -cand.return_z and 
                    abs(prev_zoi) >= cand.oi_z and 
                    prev_ztk <= -cand.taker_z and 
                    prev_zfd <= 0.0):
                    in_pos = True
                    pos_side = 'LONG'
                    entry_price = curr_open
                    entry_idx = t
                # 2. Señal SHORT: Retorno extremadamente alcista + Shock de OI + Taker Buy masivo + Funding no bajista
                elif (prev_zr >= cand.return_z and 
                      abs(prev_zoi) >= cand.oi_z and 
                      prev_ztk >= cand.taker_z and 
                      prev_zfd >= 0.0):
                    in_pos = True
    ...
    def validate_candidate(self, cand: DerivativesShockCandidate) -> DerivativesShockEvaluationResult:
        ...
        return DerivativesShockEvaluationResult(
